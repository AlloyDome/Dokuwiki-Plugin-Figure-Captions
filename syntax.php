...
class syntax_plugin_figcap extends DokuWiki_Syntax_Plugin {
	...
	public function connectTo($mode) {
		$this->Lexer->addEntryPattern('\[\{\{[^\|\}]+\|*(?=[^\}]*\}\}\])', $mode, 'plugin_figcap');
	}

	public function postConnect() {
		$this->Lexer->addExitPattern('\}\}\]', 'plugin_figcap');
	}
	...
	public function render($mode, Doku_Renderer $renderer, $data){
		...
		if ($mode == 'xhtml') {
			...
			switch ($state) {
				case DOKU_LEXER_ENTER: {
					$renderer->doc .= '<div class="thumb2 t' . $match['align'] . '"><figure class="thumbinner">';
					if ($match['exist']) {
						$renderer->{$match['type']}(
							$match['src'],
							$match['title'],
					...
					$renderer->doc .= '<figcaption class="thumbcaption" style="max-width: ' . ($match['width'] - 6) . 'px">';
					if ($match['detail']) {
						$renderer->doc .= '<div class="magnify">';
						$renderer->doc .= '<a class="internal" title="' . $this->getLang('enlarge') . '" href="'.$match['detail'] . '" target="_blank">';
						$renderer->doc .= '<img width="15" height="15" alt="" src="' . DOKU_BASE . 'lib/plugins/figcap/magnify-clip.svg"/>';
						$renderer->doc .= '</a></div>';
					}
					break;
				} case DOKU_LEXER_UNMATCHED: {
